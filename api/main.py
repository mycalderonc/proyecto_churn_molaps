...
def resumen_metricas() -> dict:
    ...
    with metricas_lock:
        ...
        tasa_error = (
            (
                metricas["errores_validacion"]
                + metricas["errores_internos"]
            )
            / total * 100
            if total
            else 0
        ...
        porcentaje_alto_riesgo = (
            metricas["predicciones_alto_riesgo"]
            / metricas["predicciones_validas"]
            * 100
            if metricas["predicciones_validas"]
            else 0
         )

        porcentaje_anomalias = (
            metricas["solicitudes_con_anomalias"]
            / metricas["predicciones_validas"]
            * 100
            if metricas["predicciones_validas"]
            else 0
        ...
        return {
            ...
            "porcentaje_alto_riesgo": round(
                porcentaje_alto_riesgo, 2
            ),

            "porcentaje_anomalias": round(
                porcentaje_anomalias, 2
            ),
...
def predict(datos: ClienteEntrada) -> PrediccionSalida:
    ...
    try:
        ...
        with metricas_lock:
            ...
            if etiqueta == "Alto_Riesgo":
                metricas["predicciones_alto_riesgo"] += 1
            else:
                metricas["predicciones_bajo_riesgo"] += 1
